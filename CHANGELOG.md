...
- 修复: 已知文件名驼峰命名
- 修复: VLC 原生输出窗口句柄校验，避免无效或跨平台不兼容的句柄进入 libVLC
- 修复: VLC 实例在页面刷新或窗口关闭后未从全局索引移除
- 修复: VLC 全屏请求失败时状态错误，并清理播放器销毁后的全屏监听
- 修复: XGPlayer 弹幕发送插件未注册，并完善回车发送、输入校验和失败处理
- 修复: WebSocket 自动重连被取消器提前禁用，以及心跳超时器误关健康连接
- 修复: 数据库同步期间的后续变更可能未被处理
- 优化: VLC 播放指标改为单次批量 IPC，并避免慢请求重叠
- 变动: 依赖库升级
